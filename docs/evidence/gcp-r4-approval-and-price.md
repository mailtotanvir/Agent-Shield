# GCP r4 Approval and Price Quote
...
- Dedicated cluster and repository: `agentshield-evidence-r4`.
...
- One image build and one diagnostic/evidence attempt.
- 25-minute watchdog with automatic teardown on success, error, signal, or
  timeout.
- Gross list-price cap: **$0.25 USD**, excluding tax and ignoring free tiers.

## Fresh Google Cloud Billing Catalog quote

Queried from the official catalog at `2026-09-19T04:09:03Z`. Prices were
...
used to reduce the estimate.

## Diagnostic additions

Before ingestion, r4 verifies the broker's effective Google service-account
email without retaining its access token. KMS failures preserve only their
bounded exception class. Job polling exits immediately on a Failed condition.
All other r3 network, readiness, evidence, and teardown controls remain active.

## Outcome

The approved one-shot run completed and was torn down. It proved the expected
broker Google service-account identity and isolated the 503 to an
`AttributeError` in the client-side response handler. Google Cloud KMS
`DecryptResponse` returns `plaintext` and `plaintext_crc32c`; unlike
`EncryptResponse`, it does not return `verified_ciphertext_crc32c` or
`verified_additional_authenticated_data_crc32c`. The adapter incorrectly read
those absent fields after a successful provider call. The correction was made
and validated on OCI. See `gcp-r4-summary.md`.
