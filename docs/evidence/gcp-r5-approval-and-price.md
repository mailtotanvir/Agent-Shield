# GCP r5 Approval and Price Quote
...
- Dedicated cluster and repository: `agentshield-evidence-r5`.
...
Queried from the official catalog at `2026-09-19T04:31:46Z`. Prices were
...
used to reduce the estimate. The sole purpose of r5 is to validate the corrected
Cloud KMS `DecryptResponse` handling and complete the authorized delivery proof.

## Outcome

The one approved attempt stopped before AgentShield deployment because the
in-cluster Kubernetes API probe exited during early cluster convergence. Calico
reported Ready at the gate but immediately replaced its initial node and Typha
pods while the probe was starting. The probe's failure log was not captured by
this runner, so the exact Python exception is unknown and no stronger claim is
made. Automatic teardown completed; see `gcp-r5-summary.md`.
