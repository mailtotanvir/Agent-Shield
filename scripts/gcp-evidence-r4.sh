...
readonly CLUSTER="agentshield-evidence-r4"
readonly REPOSITORY="agentshield-evidence-r4"
...
readonly IMAGE_TAG="gcp-r4"
readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
readonly EVIDENCE_DIR="${ROOT_DIR}/docs/evidence/gcp-r4"
readonly RUN_DIR="$(mktemp -d /tmp/agentshield-gcp-r4.XXXXXX)"
...
if [[ "${AGENTSHIELD_GCP_APPROVAL:-}" != "approved-r4-lifecycle" ]]; then
  echo "Refusing mutation. Set AGENTSHIELD_GCP_APPROVAL=approved-r4-lifecycle." >&2
  exit 2
fi

if [[ -e "${EVIDENCE_DIR}/started-at.txt" ]]; then
  echo "Refusing to repeat the completed r4 approval; create a newly approved revision." >&2
  exit 2
fi
...
mutated=true
(sleep "$MAX_RUNTIME_SECONDS"; echo "r4 watchdog reached 25 minutes; starting teardown" >&2; kill -TERM "$$") &
watchdog_pid=$!
        ...
        - name: ingest
          ...
          args:
            - |
              ...
              value = bytearray(b"agentshield-gke-r4-" + secrets.token_bytes(32))
              try:
...
if rg -l 'agentshield-gke-r4-' "$EVIDENCE_DIR" >/dev/null; then
