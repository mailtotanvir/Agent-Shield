...
readonly CLUSTER="agentshield-evidence-r5"
readonly REPOSITORY="agentshield-evidence-r5"
...
readonly IMAGE_TAG="gcp-r5"
readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
readonly EVIDENCE_DIR="${ROOT_DIR}/docs/evidence/gcp-r5"
readonly RUN_DIR="$(mktemp -d /tmp/agentshield-gcp-r5.XXXXXX)"
...
if [[ "${AGENTSHIELD_GCP_APPROVAL:-}" != "approved-r5-lifecycle" ]]; then
  echo "Refusing mutation. Set AGENTSHIELD_GCP_APPROVAL=approved-r5-lifecycle." >&2
  exit 2
fi

if [[ -e "${EVIDENCE_DIR}/started-at.txt" ]]; then
  echo "Refusing to repeat the completed r5 approval; create a newly approved revision." >&2
  exit 2
fi
...
mutated=true
(sleep "$MAX_RUNTIME_SECONDS"; echo "r5 watchdog reached 25 minutes; starting teardown" >&2; kill -TERM "$$") &
watchdog_pid=$!
        ...
        - name: ingest
          ...
          args:
            - |
              ...
              value = bytearray(b"agentshield-gke-r5-" + secrets.token_bytes(32))
              try:
...
if rg -l 'agentshield-gke-r5-' "$EVIDENCE_DIR" >/dev/null; then
