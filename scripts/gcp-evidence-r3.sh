...
readonly CLUSTER="agentshield-evidence-r3"
readonly REPOSITORY="agentshield-evidence-r3"
...
readonly IMAGE_TAG="gcp-r3"
readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
readonly EVIDENCE_DIR="${ROOT_DIR}/docs/evidence/gcp-r3"
readonly RUN_DIR="$(mktemp -d /tmp/agentshield-gcp-r3.XXXXXX)"
readonly KUBECTL_BIN="${KUBECTL_BIN:-/tmp/agentshield-tools/kubectl}"
readonly HELM_BIN="${HELM_BIN:-/tmp/agentshield-tools/helm}"
readonly MAX_RUNTIME_SECONDS=2700
readonly APPROVED_GROSS_CAP_USD="0.35"
readonly QUOTED_MAX_GROSS_USD="0.2520"
...
if [[ "${AGENTSHIELD_GCP_APPROVAL:-}" != "approved-r3-lifecycle" ]]; then
  echo "Refusing mutation. Set AGENTSHIELD_GCP_APPROVAL=approved-r3-lifecycle." >&2
  exit 2
fi

if [[ -e "${EVIDENCE_DIR}/started-at.txt" ]]; then
  echo "Refusing to repeat the completed r3 approval; create a newly approved revision." >&2
  exit 2
fi
...
mutated=true
(sleep "$MAX_RUNTIME_SECONDS"; echo "r3 watchdog reached 45 minutes; starting teardown" >&2; kill -TERM "$$") &
watchdog_pid=$!
...
with urllib.request.urlopen(
    ...
    if response.status != 200:
        raise SystemExit(f"unexpected Kubernetes API status: {response.status}")
print("in-cluster Kubernetes API connectivity verified")
'
"$KUBECTL_BIN" --namespace "$AGENT_NAMESPACE" wait --for=jsonpath='{.status.phase}'=Succeeded \
  pod/kubernetes-api-connectivity --timeout=120s
...
print(getattr(credentials, "service_account_email", "unknown"))
        ...
        - name: ingest
          ...
          args:
            - |
              ...
              value = bytearray(b"agentshield-gke-r3-" + secrets.token_bytes(32))
              try:
...
if rg -l 'agentshield-gke-r3-' "$EVIDENCE_DIR" >/dev/null; then
